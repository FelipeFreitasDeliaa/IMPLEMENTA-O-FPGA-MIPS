module DATAPATHGERAL (
    ...
    assign fio_reg2_banco = (fio_r2esp == 1'b1) ? fio_reg2 : fio_mux_rd_rs_rt;
	  ...
	  // IN_ativo: verdadeiro quando a instrução atual é IN
		wire fio_in_ativo = (fio_opcode == 6'b100001);

	 // Stall: IN executando e botão ainda não pressionado
      wire fio_stall = fio_in_ativo & ~BOTTON;

   // RegWrite gateado: durante IN, só escreve em RIN quando botão for pressionado
      wire fio_regwrite_gateado = fio_in_ativo ? (fio_regwrite & BOTTON) : fio_regwrite;
    ...
    .clk            (clk),
    .HLT            (fio_hlt),
    .Stall          (fio_stall),          // NOVO
    .Branch         (fio_branch),
    .z_flag         (fio_z_flag),
    .Jump           (fio_jump),
    .JR             (fio_jr),
    .Imediato_Branch(fio_imediato_estendido),
    .Imediato_Jump  ({6'b000000, fio_campo_26}),
    .Endereco_JR    (fio_dado1_out),
